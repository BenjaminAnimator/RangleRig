...
from RangleRig.toolkit import pvCalculator
from RangleRig.toolkit import setDvrKey

class armGen():
    def __init__(self, topjoint, characterName,rigGrp, settingsGrp, visGrp):
        ...
        baseJoints = selectHirearchy.jntHirearch(jnt1,True)
        

        
        
        fkGrp = mc.duplicate(baseJoints[0],name = 'Fkjnt', renameChildren = True)
        ikGrp = mc.duplicate(baseJoints[0],name = 'ikjnt', renameChildren = True)
        
        armParts = ['Uparm', 'Loarm', 'Hand']
        ...
        armIkHandle= mc.ikHandle(name = prefix +'arm_ik_handle', startJoint = ikJnts[0] , endEffector =ikJnts[2], solver = 'ikRPsolver' )
        
        polVector = pvCalculator.pvCal(ikJnts[0], ikJnts[1], ikJnts[2], prefix + ID + 'arn_polvec')
        ...
        #Create Vis Control
        mc.select(visGrp)
        mc.addAttr( shortName=ID + '_ArmVis', longName=ID + '_ArmVis', attributeType = 'enum', enumName = 'FK:IK:Both:None' , keyable = True, hidden = False )
        
        #Set Visibility Keys Left Legs
        for i in fkCon:
            setDvrKey.setDvrK(visGrp +  '.' + ID + '_ArmVis', i + 'Shape.visibility', 0, 1)
            setDvrKey.setDvrK(visGrp +  '.' + ID + '_ArmVis', i + 'Shape.visibility', 1, 0)
            setDvrKey.setDvrK(visGrp +  '.' + ID + '_ArmVis', i + 'Shape.visibility', 2, 1)
            setDvrKey.setDvrK(visGrp +  '.' + ID + '_ArmVis', i + 'Shape.visibility', 3, 0)
        
        for i in ikCon:
            setDvrKey.setDvrK(visGrp +  '.' + ID + '_ArmVis', i + '.visibility', 0, 0)
            setDvrKey.setDvrK(visGrp +  '.' + ID + '_ArmVis', i + '.visibility', 1, 1)
            setDvrKey.setDvrK(visGrp +  '.' + ID + '_ArmVis', i + '.visibility', 2, 1)
            setDvrKey.setDvrK(visGrp +  '.' + ID + '_ArmVis', i + '.visibility', 3, 0)
